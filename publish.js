...
function buildNav(members, navTypes = null, betterDocs) {
    ...
    types.forEach(function (type) {
        ...
        members[type] = ['namespaces'].includes(type) ? members[type] : members[type].sort((a, b) => ('' + a.name).localeCompare(b.name))
...
exports.publish = function (taffyData, opts, tutorials) {
    ...
    if (fs.existsSync(path.join(opts.destination, '..', 'sri-hash.txt'))) {
        const integrity = fs.readFileSync(path.join(opts.destination, '..', 'sri-hash.txt'), 'utf8');
        conf.betterDocs.integrity = integrity;
    }
